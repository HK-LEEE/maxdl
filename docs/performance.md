...
> **상태(2026-05-21)**: 1순위 Trino 튜닝 = **적용 완료**
> (`charts/trino/values.yaml` 활성, helm rev 16). 본 문서의 §2 값표가
> 이제 라이브 구성이고, §3 절차는 미래 추가 튜닝 또는 롤백 시 참고.
> 롤백 경로: `helm -n maxdl-query rollback trino 14` (튜닝 전 리비전).
> 차후 튜닝 라운드는 같은 사이드카 패턴(`values.yaml_bak` 신규 작성
> → §3 절차로 활성화)으로 재사용 가능.
