...
## 2. `schedule` 에 들어갈 수 있는 값

`ingestion-map.yaml` 의 `schedule:` 은 **문자열**로 받는다. YAML 에서
실제로 사용 가능한 형식은 두 가지(프리셋·cron 표현식)뿐이며, 그 외는
현재 구조에서 직접 지정 불가(아래 §2-3 제한 표).

### 2-1. 사용 가능한 형식

| 형식 | 예시 | 의미 (KST — §3 타임존 적용 후) |
|---|---|---|
| 프리셋 | `"@once"` | 1회만 |
| 프리셋 | `"@hourly"` | 매시 0분 |
| 프리셋 | `"@daily"` | 매일 00:00 KST |
| 프리셋 | `"@weekly"` | 매주 일요일 00:00 KST |
| 프리셋 | `"@monthly"` | 매월 1일 00:00 KST |
| 프리셋 | `"@yearly"` / `"@annually"` | 매년 1/1 00:00 KST |
| cron | `"0 */2 * * *"` | 2시간마다 |
| cron | `"*/30 * * * *"` | 30분마다 |
| cron | `"0 3 * * *"` | 매일 03:00 KST |
| cron | `"0 18 * * *"` | 매일 18:00 KST |

cron 5필드 = `분 시 일 월 요일`. 특수문자: `*`(전체) `,`(목록) `-`(범위)
`/`(스텝).

### 2-2. 구체 예시 (제조 운영 컨텍스트)

| 케이스 | 추천 schedule | 비고 |
|---|---|---|
| 설정성 마스터(거의 안 변함) | `"@daily"` / `"@weekly"` | replica 비중 큰 소스 |
| 트랜잭션성 (시간 단위 분석) | `"0 * * * *"` (매시) 또는 `"0 */2 * * *"` | merge 위주 권장 |
| 야간 일괄 분석 (KST 새벽 운영) | `"0 3 * * *"` | 새벽 03:00 KST 일괄 |
| 평일만 가동(공장 운영일) | `"0 3 * * 1-5"` | 토·일 스킵 |
| 교대 시간 직후 집계 (08·16·24) | `"5 0,8,16 * * *"` | 5분 버퍼 후 |
| 월말 마감 | `"0 23 28-31 * *"` | 매월 말일 가까이(croniter 가 실재일 처리) |
| 빈도 매우 높음(라이브 모니터링) | `"*/15 * * * *"` | merge 전제, replica 대형 테이블이면 금물 |

### 2-3. 사용 불가 / 제한 (현재 구조)

| 형식 | 가능? | 사유 |
|---|---|---|
| `timedelta(hours=6)` (Python 객체) | ❌ | YAML 은 문자열만 — 코드 변경 필요(현재 미지원) |
| `null` / `~` (= 수동 트리거만) | ⚠ **차단** | 팩토리가 "schedule 누락 시 즉시 실패" 가드(fallback 없음). 의도된 강제 |
| `[Asset(...), ...]` (Dataset 트리거) | ❌ | Python 객체. Gold 변환 DAG 만 코드에서 사용(자동) |
| `"@continuous"` | △ 작동은 함 | 직전 실행 종료 즉시 재시작. 부하 매우 큼 — 거의 사용 X |

> **수동 트리거만** 으로 운영하려면(예: 백필 전용 DAG) 팩토리 가드를
> 일부 완화해야 함(별도 PR 사안). 현 정책은 모든 소스에 schedule 명시.

### 2-4. 작성 팁

- **반드시 따옴표**로 감싼다: `"0 */2 * * *"` (별표·콜론이 YAML 특수
  문자라 미따옴 시 파싱 오류 가능).
- **검증**: cron 의미는 crontab 도구로 사전 확인. 적용 후엔 `airflow
  dags next-execution ingest_<소스>` 로 다음 실행 시각(KST 표시) 확인.
- **모드 ↔ 주기**(§4 도 참조): `replica`(전체 교체) 위주 소스에 짧은
  주기 금물. `merge`(증분) 위주는 짧게 가도 OK.
