...
운영이 명시 요구). 신원=maxplatform SSO(JWT, 사내망), 머신토큰 미발급 →
사람=JWT/기계(dbt)=PASSWORD(svc-dbt) 다중 인증자.

- **1단계 완료(코드화·검증, 라이브 미적용)**: `charts/trino/values.yaml`
  에 `authenticationType: jwt,password` + JWT props 전부 `${ENV:MAXDL_SSO_*}`
  환경변수화(`trino-sso` secret, secrets-spec/env.example 반영) +
  `allow-insecure-over-http=true`(사내망, TLS 는 FU-7). `helmfile template`
  exit 0 렌더 검증. **라이브 Trino 무변경** — 지금 켜면 토큰 없는 Superset/
  dbt 즉시 차단되므로 의도적 보류.
- **컷오버 전제(동시 충족 필요)**: ① Trino 다중인증 ② svc-dbt
  password-db(htpasswd, `SVC_DBT_PASSWORD`→Secret 생성) + Airflow/dbt 프로필
  주입 ③ Superset SSO 로그인 + Trino 임퍼소네이션. 셋 다 준비돼야 1회
  조율 컷오버.
- **남은 단계**: 2단계 dbt password-path 배선 · 3단계 Superset SSO+
