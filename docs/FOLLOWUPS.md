...
- **메타DB 영속 정석(실 k3s) 코드/문서화 완료**: `deploy/k8s/
  storageclass-retain.yaml`(maxdl-retain, rancher.io/local-path,
  reclaimPolicy=Retain) + helmfile 1단계 presync 적용 + AIRGAP_RUNBOOK
  §2-1b 절차(전용 디스크 `/data/maxdl-pv` ← local-path-config, maxdl-retain
  default 승격, 백업 2곳). 번들 차트 values 무수정으로 전 PVC 영속화.
  라이브(k3d) 무손상 — 비-default SC 라 무해, 실 k3s 클린 생성 시 적용.
