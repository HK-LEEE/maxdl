-- 자동생성: maxplatform.user_permissions Silver (merge: PK 최신 dedup, incremental)
{{ config(materialized='incremental', incremental_strategy='merge',
    unique_key=['id'], on_schema_change='append_new_columns') }}
with src as (
    select * from {{ ref('stg_maxplatform__user_permissions') }}
    {% if is_incremental() %}
...
select {{ dbt_utils.star(from=ref('stg_maxplatform__user_permissions')) }}
